...
interface PointsDisplayProps {
  ...
  compact?: boolean;
  /** Optional streak value to display alongside points */
  streak?: number;
}
...
export default function PointsDisplay({ points, compact = false, streak }: PointsDisplayProps) {
    ...
    return (
      ...
      >
        ...
        {typeof streak === 'number' && (
          <span className="ml-2 text-xs text-emerald-200/90 bg-black/10 px-2 py-0.5 rounded-full flex items-center gap-2">
            <svg viewBox="0 0 24 24" className="w-3 h-3 text-emerald-300" fill="none" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M12 8v4l3 3" /></svg>
            <strong className="text-emerald-200">{streak}</strong>
          </span>
        )}
      </motion.div>
    );
    ...
    >
      ...
      {typeof streak === 'number' && (
        <div className="flex-shrink-0 text-right">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-black/10 border border-white/6 text-sm text-emerald-300 font-semibold">
            <svg viewBox="0 0 24 24" className="w-4 h-4 text-emerald-300" fill="none" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M12 8v4l3 3" /></svg>

 ...
 *   https://raw.githubusercontent.com/space-gen/aurora/refs/heads/data/data/{taskType}.json
 *
...
const BASE_URL =
  'https://raw.githubusercontent.com/space-gen/aurora/refs/heads/data/data';
...
function toEmbeddableImageUrl(url: string): string {
  ...
  // Force HTTPS for any http:// URL
  if (url.startsWith('http://')) {
    url = url.replace(/^http:\/\//i, 'https://');
  }

  // Aurora may reference JSOC hosts; prefer the embeddable jsoc1 host
  if (url.startsWith('https://jsoc.stanford.edu/')) {
    return url.replace('https://jsoc.stanford.edu/', 'https://jsoc1.stanford.edu/');
  }
